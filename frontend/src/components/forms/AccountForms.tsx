...
import { companySchema,companySetupSchema,memberSchema,platformRoleSchema } from '@/lib/validations/company.schema';
...
export function PlatformRoleForm({id,onClose}:{id:string;onClose:()=>void}){const {data,save,notify}=useWorkspace();const member=data.members.find(m=>m.id===id)!;const form=useForm<z.infer<typeof platformRoleSchema>>({resolver:zodResolver(platformRoleSchema),defaultValues:{role:member.platformRole}});return <FormProvider {...form}><form noValidate onSubmit={form.handleSubmit(v=>{save('members',{...member,platformRole:v.role});notify('Platform role updated in demo data.');onClose();})}><p className="form-note">{member.name} · Platform permissions are separate from company roles.</p><Field name="role" label="Select Target Platform Role"><option value="USER">USER — Company assignments</option><option value="SUPER_ADMIN">SUPER_ADMIN — Platform administration</option></Field><FormActions onCancel={onClose} label="Update Role"/></form></FormProvider>;}
